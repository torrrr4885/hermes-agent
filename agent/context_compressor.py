...
class ContextCompressor(ContextEngine):
    ...
    def __init__(
        ...
        # When True (default for this runtime branch), summary-generation failure
        # aborts compression entirely (returns messages unchanged, sets
        # _last_compress_aborted=True).
        # When False (explicit opt-out), insert a deterministic "summary
        # unavailable" handoff and drop the middle window.
...
The user has requested that this compaction PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        # If LLM summary failed, fail closed by default on this runtime branch.
        # Do not replace the middle of the transcript with an unsummarized
        # placeholder: that preserves token space but loses the user's actual
        # history. Operators can explicitly set
        # compression.abort_on_summary_failure=false to use the deterministic
        ...
        if not summary and self.abort_on_summary_failure:
            ...
            if not self.quiet_mode:
                logger.warning(
                    "Summary generation failed — aborting compression to preserve "
                    "%d message(s). Conversation is unchanged until /compress "
                    "succeeds or /new starts a clean session.",
                    n_preserved,
                )
        ...
        # Legacy opt-out fallback path: when abort_on_summary_failure is False,
        # insert a deterministic summary so the model gets locally recoverable
        # continuity anchors instead of a content-free marker.
        if not summary:
            if not self.quiet_mode:
